...
方法：六路并行审计（隧道与进程 / MainActivity 与生命周期 / 终端与一致性测试台 / 凭据·通知·诊断 /
注入插件 / CI 与验证脚本）+ 通读 `MainActivity`、`DshApp`。每条结论都回读代码，少数几条做了实测
...
| M16 | `dsh-handheld-mobile.js`（turn watcher 的 `if (ms < MIN_TURN_MS) return`）、`DshApp.kt:133/137/175` | `<1.5s` 的轮次只发 `turn-start`，`running=false` 是页面局部状态 → App 侧 `pageBusy` 永远停在 true | `MainActivity.onPause` 永远走「保留定时器」分支（后台白耗电），「空闲即暂停」静默失效 | 短轮次也补一条 `turn-done`（由 App 决定要不要通知） |
