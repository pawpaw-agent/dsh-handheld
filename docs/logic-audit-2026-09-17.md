...
### 真机再定案：通知为什么「有时有、有时没有」—— renderer 在后台被系统冻掉

第三、四次真机实验（rev 1.0.23）：

- **短后台（退到后台 19s 后这一轮结束）** → 通知正常：`21:29:05 前台=false` →
  `DshNotifier: 已发任务完成通知` → 通知栏 `id=2 channel=dsh-turn-hi importance=4` ✓。
- **长后台（退到后台 6 分钟以上，期间这一轮一直在流式输出）** → **一条心跳都没有**
  （最后一条是 21:29:05），也就是说页面**根本没在渲染** —— 观察者既没漏报也没错报，它没被调用。
  同一窗口里那条每 5 分钟的 `turn-tick` 存活探针也没出现（Chromium 对隐藏页面的定时器节流），
  所以「用定时器兜底」这条路也不通。
- 之前的失败（21:17 那次，3 分钟量级）与此完全一致：`21:20:10` 一回到前台，页面**直接**变成
  「当前这轮在跑」，中间那次结束从未出现在 DOM 里。

机理：WebView 的渲染是**独立子进程**（`dumpsys activity processes` 里的
`SandboxedProcessService`，不可见时状态就是 `WPRI` = waived priority）；它被 waive/冻结后
JS 停跑、DOM 不再更新、定时器被节流。触发阈值只有几十秒，于是表现成「有时候有通知、有时候没有」。

已做的两处（rev 1.0.23）：`setRendererPriorityPolicy(RENDERER_PRIORITY_IMPORTANT, false)`（要求
「不可见也别放弃这个 renderer」）、`onPause` 在通知开关开着时保留定时器。**实测这两处并没有
让长后台的页面活下来** —— 所以真正的结论是：

> **不能用页面当「这一轮结束了」的信号源。** 要可靠，信号必须来自页面之外（App 自己的进程通过
> 隧道/SSH 问服务端会话状态）。这条还没做 —— 需要先找到「这个会话在不在跑」的服务端入口，
> 并让页面在 turn-start 时把会话 id 告诉 App（新的契约钩子）。
