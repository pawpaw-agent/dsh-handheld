...
用户：「左侧边栏不要隐藏按钮了」。数墨点（抽屉打开时，两颗头部按钮各自区域内的深色像素）：

```
头部左按钮区域(x 11..43 css, y 11..45 css) =     0 个墨点   ← 被抽屉整颗盖住
头部右按钮区域(x 344..373 css, 同上)       = 14300 个墨点  ← 照样看得见
```

抽屉一开，左边那颗「消失」、右边那颗还在 —— 正是他一直在报的**左右不对称**的另一种形态。
改法两步：

1. 头部提到**抽屉与遮罩之上**（`z-index: 45` > 抽屉的 40）——那颗目录按钮照旧在，
   点它就能把抽屉收回去（同一颗按钮开/关，不再是「打开后就找不到了」）；
2. 抽屉的内容下移到头部底边之下（头部有实底背景，不能让抽屉的 logo 行钻进去）。
   「头部底边」**不写死**：外壳 padding-top + 标题行 + 页签都是内容撑的（上次目录按钮
   `top` 写死 12px 就差 10px），改由插件量 `getBoundingClientRect().bottom` 写进 frame 的
   `--dsh-handheld-head-bottom`（首帧 + 400ms + 1500ms + resize + ResizeObserver），
   `layout-diag` 顺势把该变量报回来。

真机复核（rev 1.0.36）：变量 = **71px**；抽屉打开时左按钮墨点 **0 → 1208**、几何与关闭时
完全一致（`x 18.1..33.9, y 20.5..35.2`，中心 y 27.9）；再点一次那颗按钮，遮罩与抽屉一起消失
（左右 300px 平均亮度 244.9/255，恢复未遮罩）。
