...
## 性能：适配层的常驻开销在哪（1.0.64 – 1.0.68，2026-09-22/24）
...
1. **负向查询限流**：轮次监视和待输入监视在「节点不在」时，**每个 mutation 批次**都要做一次全树
   `querySelector(All)`，而「节点不在」恰恰是多数时间。改法是给**未命中**的查询加 ≥250ms 地板
   （判「结束」不受影响 —— 那条只看 `isConnected`，不查询）。**没做**：它需要真机数据证明值得，
   也得确认没有把「开始」的发现延迟拖到可感知。这是下一步的第一顺位。
2. **合并 4 个 `documentElement`/`body` 的 subtree 观察者**成一个 + rAF 批处理，各消费者挂脏标记。
   收益明确但会动到轮次监视那套来之不易的语义（心跳、`pick` 的挑法），**没做**。
3. **行内操作改成事件委托**（一个文档级 `click` + `closest('[role=treeitem]')`），可以整块删掉 2N 次扫描。
   **没做**：走的是宿主自己的菜单路径，必须真机验证过才敢换。
4. **顶栏 sticky 的帧开销**：归因不可信（见上），且「要不要只让键盘弹起时才 sticky」是产品选择。
5. **110 处 `!important`**：这是可维护性问题，不是运行时问题，不为了数字好看去动它。
6. **给诊断加开关**（`window.__dshHandheldDebug`，由 App 写入）：一次性诊断本身很便宜，收益主要是日志
   体积，不是帧率。
...
当前为 `dsh-handheld-mobile-1.0.68`。

> 1.0.68 = 只改注释：撤回 1.0.67 里那句错误的「顺带修掉一个语义错」（真机 right-probe 取证，见性能一节），行为与 1.0.67 完全一致；1.0.67 = 面板遮挡的开合判据前置到读几何之前（收起时零强制布局）＋面板元素缓存；1.0.66 = 9 条文档级
