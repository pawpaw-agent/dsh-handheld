...
## 性能：适配层的常驻开销在哪（1.0.64 – 1.0.67，2026-09-22/23）
...
1.0.67 还顺带修掉一个语义错：`visibility: hidden` 的元素**仍然有盒**，所以收起状态的
`data-sidebar-right-panel=fullscreen` 面板会被旧判据算成「展开」—— 对着一个用户看不见的面板扫遮挡。
几何判据没有整个删掉：属性只说**该**展开，滑入动画那 ~300ms 里几何还没落定，此时采样点会打偏到面板外
命中无关元素，所以「落定」仍旧只认几何。

### 真机读数与**它的可信度**

30s 连续滚动，`dumpsys gfxinfo`（设备 1 = SM-S9280 / Android 16，约 1020 帧）：

| rev | janky | 99th | missed vsync |
|---|---|---|---|
| 1.0.52（targetSdk 35 基线） | 1.24% | 10ms | 0 |
| 1.0.58 | 0.95% | 8ms | 0 |
| 1.0.60（只加了顶栏 sticky） | **6.51%** | 25ms | 5 |
| 1.0.64 | 0.77–2.04% | 12–16ms | 0–2 |
| 1.0.65 | 0.98% | 9ms | 1 |

> ⚠️ **这批数字只够看趋势，不够做归因。** 同一个包重跑可以在 3.57% ↔ 7.61% 之间跳（短采样的方差更大），
> 所以「sticky 顶栏让 1.0.60 掉到 6.51%」**没有被证实** —— 它既可能是真回归，也可能是这一次采样的运气。
> 要下结论得固定协议：同一个 30s 脚本、同一段内容、**每个包重复 ≥3 次取中位数**。
> 1.0.66/1.0.67 的改动落在这里的**静态依据 + 插件自报耗时**上（强），**不是**上面这张表的差值（弱）。
...
当前为 `dsh-handheld-mobile-1.0.67`。

> 1.0.67 = 面板遮挡的开合判据前置到读几何之前（收起时零强制布局）＋面板元素缓存；1.0.66 = 9 条文档级
