...
`:hover`/`menuOpen` 才 `display:inline-flex` —— 窄屏必须一律显示；真机 340px 抽屉里标题仍剩 245px ✓）。
...
当前为 `dsh-handheld-mobile-1.0.38`。最近的几档：1.0.38 = 会话行的 `⋯` 也不被 hover 藏起来
