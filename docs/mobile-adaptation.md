...
真机上差 10.1px，用户一眼看到「两边上下不对称」）；**宿主把按钮藏在 hover 里 = 手机上永远没有**（工作区行的 `⋯`/`+` 只有
`:hover`/`menuOpen` 才 `display:inline-flex` —— 窄屏必须一律显示）。
...
当前为 `dsh-handheld-mobile-1.0.37`。最近的几档：1.0.37 = 工作区行上的 `⋯`/`+` 不再被 hover 藏起来
