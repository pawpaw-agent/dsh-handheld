...
真机上差 10.1px，用户一眼看到「两边上下不对称」）；**抽屉不许盖住头部**（`z-index`：头部 45 > 抽屉 40 >
遮罩；抽屉的 `padding-top` 用插件量出来的头部底边 —— 真机实测被盖住时左按钮墨点 0、右按钮 14300）。
...
当前为 `dsh-handheld-mobile-1.0.36`。最近的几档：1.0.36 = 抽屉打开时头部提到它之上（左按钮不再被盖掉，
头部底边由插件量出来写进 `--dsh-handheld-head-bottom`）；1.0.35 = 头部左右按钮的竖直对齐（目录按钮的
