...
3. **把行尾那颗 `⋯` 摊平**（用户连着两次：「把 … 里面的按钮都展开」→「…里面的按钮直接展开」；
   他前一条自己写明了「工作区文件夹上的 … 和 + 按钮」，所以这两个「…」就是那颗 `⋯`）：
   重命名/删除工作区、重命名/分叉/归档会话全藏在它的菜单里，手机上要多点一次才看得到。
   改法：插件在每行的动作区里**注入**与菜单项一一对应的按钮（图标 + `aria-label`），
   点击时**替用户走一遍宿主自己的菜单**（先点 `⋯` 打开，再点对应序号的 `[role=menuitem]`）——
   菜单项在 `client/ui-workspace` 里带稳定的 id 与顺序（`workspaceMenuItems` = rename, delete；
   `sessionMenuItems` = rename, fork, archive），按序号点不会串。
   **故意不绕过宿主直接调 API**：rename/delete 走的是宿主原生的改名编辑器与删除确认框，
   绕过去等于把「确认」这一环拆掉。注入成功才把 `⋯` 藏起来；菜单 600ms 内没弹出来就把
   `⋯` 放回来 —— 任何一步失败都不留死路。
   真机验证：点会话行的 ✎ → 宿主弹出「重命名会话」并带出当前标题（选中待改）✓，× 关掉无副作用 ✓。
