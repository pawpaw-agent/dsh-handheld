...
ScrollView（可滚动）
  ...
  连接设置卡   折叠：user@host · 密码（非默认端口才多一段）
               展开：全部输入框 + 就地报错
贴底动作区（不随内容滚动）
  状态条       全局提示，一枚**居中的胶囊**（就地错误在字段下面）：
               一次性回执 5 秒自隐；失败原因常驻；空着也**占位**（主按钮不跳）
...
### 0.1.12：底部状态条（上面那张图里贴底区的最上面那一条）

它是连接屏唯一的**全局提示位**：字段级错误走输入框下面那几行，它只管「刚才那一下怎么样了」。
真机用下来有三个毛病，都不大，但每次都硌一下 —— 三个都能只读代码复现：

| # | 毛病 | 证据 | 改法 |
|---|---|---|---|
| 1 | **术语泄漏 + 已经过时**：`connectWeb()` 的第一行是 `status("连接中… $url")`，而 `$url` 是隧道基址 `http://127.0.0.1:3080` | 与上一节 `heroSubtitle()` 同一个病（127.0.0.1 是**手机自己**）—— 顶部改了、底部漏了。更糟的是下一行就切到网页屏：这句只在用户按 BACK 回连接屏时才露出来，那时它已经过时 —— 底部「连接中…」与顶部「已连上电脑」同屏，自相矛盾 | 整条删掉（改成 `status("")`）。「正在连 / 正在打开」由顶部状态块 + ①②③ 进度行讲 |
| 2 | **一次性提示永不消失**：`status()` 只有「被下一句替换」这一种清空方式 | 勾一下「dsh 需要你时提醒我」→「已开启提醒」会一直挂在底部；断开后的「已断开」和顶部「未连接」是同一句话说两遍 | 按**寿命**分两个入口：`notice()`（一次性回执，5 秒自隐）与 `status()`（常驻：失败原因、要用户照着做的提示）。改成 `notice()` 的调用点：已开启/已关闭提醒、已开启任务完成提醒、已断开、已取消连接、正在连接中 |
| 3 | **这一块自己在动**：状态条宽度是 `MATCH_PARENT`，空/非空用 `GONE` | 3 个字的「已断开」被撑成一条整宽横条（`bg_status_pill` 本来就是胶囊形，拉满就成了一条工具栏）；提示出现/消失时主按钮上下跳一行 —— 用户正要按的时候按钮挪了位置 | 宽度跟着字走 + 水平居中；空的时候用 `INVISIBLE` 占住那一格（不是 `GONE`），主按钮位置恒定 |

同一层顺手清掉的四条文案（判据是 README「连接屏刻意去术语」）：

| 原来 | 现在 |
|---|---|
| `自动获取令牌…` | `正在重新确认网页访问权限…` |
| `自动获取令牌失败，请检查服务端 dsh-web.service` | `网页访问权限确认失败 —— 检查电脑上的 dsh 是否在运行` |
| `尚无 SSH 隧道，请先连接` | `还没连上电脑，请先连接` |
| `私钥路径为空，请到连接屏重新填写`（用户**就**在连接屏上） | `私钥路径为空，请点「导入」重新选择私钥` |

两条要记住的推论：

- §七 那张真机表里的「27 秒后截图仍是『已取消连接』」是 **0.1.10 的记录**，本版起不再复现：
  一次性回执 5 秒后自己消失（顶部状态块此时已经是「未连接」，信息没有丢）。
- **形状与寿命这两条没有真机验证过**：本地不编译（README「构建只走 CI」），只按布局与
  Handler 语义推导。装包后要看的是三件事 —— 提示出现/消失时主按钮不动、长文案
  （「这台电脑的 SSH 身份和上次不一样…」）自动折行不截断、一次性回执到点真的空了。

**没做**（各自是独立改动，不该混进「显示」这一层）：`rebuildTunnel()` 失败时那句
「重连失败…」写在**网页屏**上 —— 状态条住在连接屏里，用户当时根本看不到它；
回连接屏时（`refreshConnectState()` 只在隧道活着时清状态条）才会看到一句来自几分钟前的话。
要真的送达得换成浮层或直接切回连接屏。
