...
### 设置页「关不掉」：抽屉的「点一下收起来」把模态一起冻住了（2026-09-14，插件 rev 1.0.11）
...
| 层 | 改法 |
|---|---|
| 触发端（插件 JS） | 「点一下收起来」的目标集合排除浮层：`[role=dialog] / [role=alertdialog] / [role=menu] / [role=listbox] / [aria-modal]` 里的点击不参与这条启发式 |
...
| 动作 | 结果 |
|---|---|
| 抽屉里点「设置」→ 点对话框里一行说明文字（非按钮） | 抽屉**不再**被收起 |
| 接着点 ✕ | 对话框关闭 ✓（修复前：这一下必然无效） |
| 再开一次，直接点对话框外的遮罩 | 关闭 ✓ |
| 开着设置页按系统 BACK | 关闭 ✓（修复前会跳到连接屏、设置页还开着） |
| 抽屉里点会话行 | 仍然照旧收起抽屉并切会话 ✓（启发式本身没坏） |
