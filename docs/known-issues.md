...
ScrollView（可滚动）
  ...
  状态块       ● 已连上电脑 / 未连接 / 连接中… / 连不上你的电脑
               xsj@192.168.0.135:22 → 127.0.0.1:3080
...
- [ ] 配置完整时进入连接屏：默认**折叠**，首屏只有一个主按钮 + 一条「断开连接」
- [ ] 折叠卡摘要与保存的配置一致；点「修改 ›」展开，展开后按钮变「收起」
- [ ] 语音/读屏：输入框能被读出标签（`labelFor`），不只是「编辑框」
- [ ] 故意留空地址点连接 → 该输入框红框 + 下方红字 + 自动滚到可见位置 + 软键盘弹出
- [ ] 连接中：主按钮是「取消连接」；点它 → 状态块回「未连接」、状态条「已取消连接」，
      且**几秒后不会被在飞的那次拨号覆盖**（`connectAttempt` 代数守卫）
- [ ] 连接失败：状态块变红色「连不上你的电脑」+ ① 行红字，主按钮回到「连上并打开…」（= 重试）
- [ ] 分段控件切到「打开终端」：选中段是深字浅底（缺陷 2 的复验）
- [ ] 键盘弹起时动作区被顶到键盘上方，输入框不被遮住
- [ ] BACK：网页 → 连接屏（隧道保持）→ 再 BACK 退到后台，三级阶梯不变
