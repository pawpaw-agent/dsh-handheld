...
## 九、targetSdk 35 真机验证清单（2026-09-22）

`targetSdk` 从 34 升到 **35**。这条改动**只动一个数字**，但它打开的是 Android 15 的
**强制 edge-to-edge**（系统栏区域不再自动让位）—— 属运行时行为变更，必须真机回归。

### 为什么预期影响很小

本 App **一直是主动 opt-in edge-to-edge 的**：`MainActivity.applyImmersive()` 从第一版就调了
`setDecorFitsSystemWindows(false)`，那正是「关闭系统自动让位」本身。所以 35 强制的行为我们
早就在做，框架接手后**退化为幂等**，布局结果不变。
...
这不是留白问题 —— **终端是字符网格，被系统栏盖住就是真的看不见那几行**。

### 清单

设备要求：**Android 15（API 35）或更高**。低版本设备验不出本次变更。

#### A. 终端模式（重点，独立于主界面）

| # | 动作 | 通过判据 |
|---|---|---|
| A1 | 进终端，执行 `stty size` | 行列数与**未被遮挡的可见区域**一致 |
| A2 | 看首行提示符 | 提示符**没有被状态栏压在底下** |
| A3 | 看末行 | 最后一个提示符没被导航栏/手势条盖住 |
| A4 | 执行 `seq 1 100` 再 `clear` | 清屏后首行不乱、不叠字 |
| A5 | 弹出/收起软键盘 | 行列数（`stty size`）跟着变，且不残留空白 |
| A6 | 与 targetSdk 34 的旧包对比 `stty size` | **若行列数变化，确认是变好而不是变坏** |

> A1 是关键判据。此前真机记录是 `21 × 40`（软键盘弹出后），升级后若变成别的值，
> 要判断是「多用了被系统栏盖住的空间」（坏）还是「正确用满了整屏」（好）。

#### B. 主界面 / WebView

| # | 动作 | 通过判据 |
|---|---|---|
| B1 | 打开网页，看会话头顶部 | 顶部仍是 ~12px 压缩，**没有退回 35px 大片留白** |
| B2 | 看右侧边栏面板头 | 不被系统状态条遮挡（这是 rev 1.0.50 修过的） |
| B3 | 左抽屉展开 | 内容不被状态栏盖住（`padding-top: env(safe-area-inset-top)`） |
| B4 | 挖孔/刘海区域 | 顶部内容避开挖孔，且不多留白 |
| B5 | 滚动到底 | 最后一行内容不被手势条盖住 |

#### C. 系统栏行为（三星 ROM 重点关注）

| # | 动作 | 通过判据 |
|---|---|---|
| C1 | 进入 App | 状态栏/导航栏**隐藏**（沉浸式仍生效） |
| C2 | 从边缘上滑 | 系统栏临时出现、样式是**透明叠加**而非黑条 |
| C3 | 切后台再回来 | 沉浸式自动恢复（`onWindowFocusChanged` → `applyImmersive`） |
| C4 | 熄屏再亮 | 同上，且隧道仍活着（**顺带补 0.1.8 起一直未验的熄屏保活**） |

> C1–C3 是在验 `systemUiVisibility` 那套 deprecated 兜底标志在 targetSdk 35 下是否还有效。
> 代码注释说「部分 One UI / 老 WebView 依赖它才能真正全屏」—— 这条路径本次必须复验。

#### D. 顺带补做（审计 §一·补 要求的冒烟，一直没做）

| # | 动作 | 通过判据 |
|---|---|---|
| D1 | 网页能开 | 会话列表、工作区从服务端载入 |
| D2 | 终端能连 | 出现彩色提示符 |
| D3 | 会话日志能导出 | 走 DownloadManager，文件真的落盘 |

> D1–D3 是审计 L6/L7/L8 改了网络与注入边界（明文收窄到回环、`shouldInterceptRequest`
> 收紧到回环 origin + 插件 id、注入脚本 origin 收窄）之后**明确要求**的冒烟，至今未做。

#### E. 通知（回归，预期不受本次影响）

| # | 动作 | 通过判据 |
|---|---|---|
| E1 | 后台短退（<30s）一轮结束 | 收到横幅通知 |
| E2 | 后台长退（>6min）一轮结束 | **仍**收到通知（靠 `HarnessEventsClient` 那条路） |
| E3 | 前台一轮结束 | **不**打扰 |
...
| # | 动作 | 通过判据 |
|---|---|---|
| F1 | `aapt2 dump badging` | `targetSdkVersion:'35'` |
| F2 | 同上 | `application-debuggable` **不存在**（release 包） |

### 失败时的退路

35 还留着 `R.attr#windowOptOutEdgeToEdgeEnforcement` 可以临时 opt-out。
**但 36 会把它废弃并禁用** —— 所以退路只在 35 这一档有效，不能靠它长期拖着。

若终端那一半（A 组）不通过，**先修 `TuiActivity` 的 inset 处理再考虑升 36**：
它需要把系统栏 inset 折进 PTY 的行列数，而不只是给容器加 padding（padding 不改变
`stty size`，字符网格依然会错位）。

**2026-09-22 实测：A 组确实不通过**（`stty size` 44 行 vs 可见 28.4 行），且升 35 另带一条
新缺陷（附加键栏被软键盘盖住）——见下一节。

---

### 2026-09-22 真机回归结果（SM-S9280 / Android 16 / API 36，0.1.14）

设备 `192.168.0.175:38467`，release 包（CI run `35702958429`，versionCode 41 / targetSdk 35）。
取证目录 `/home/xsj/dsh-verify/`（全程截图 + `logcat.txt`）。下表的数字都是**量出来的**。

| 组 | 项 | 结果 |
|---|---|---|
| F1/F2 | targetSdk 35 / 无 `application-debuggable` | ✅ `versionCode=41 targetSdk=35`；grep debuggable 无输出 |
| C1 | 沉浸式（系统栏隐藏） | ✅ `vsysui=0x1706`（正是 `applyImmersive` 那 6 个标志）+ `Requested non-default-visibility types: statusBars navigationBars captionBar`；截图顶部 0–8 行零非背景像素 |
| C2 | 边缘上滑 → 临时系统栏 | ⚠️ 未能单独复现：手势导航下短滑（58px）无反应，长滑直接进桌面 / 通知栏 |
| C3 | 切后台再回来 → 沉浸式恢复 | ✅ 回前台后 `vsysui` 仍 `0x1706`；`revalidateTunnel: 探针通过，复用现有隧道` |
| C4 | 熄屏再亮 → 沉浸式 + 隧道保活 | ⚠️ 半通过，见**发现 3** |
| B1 | 会话头顶部 ~12px 压缩 | ✅ 顶部首个内容行 y=77 设备px = **20.5 CSS px**（不是 35px 那片）；窗口 `w384dp h832dp` |
| B2 | 右侧栏面板头不被状态条遮挡 | ✅ 面板首行 y=169 = **45.1 CSS px**，在挖孔带（0–34.1）之下 |
| B3 | 左抽屉内容不被状态栏盖住 | ✅ 首行 **62.9 CSS px** |
| B4 | 挖孔区域 | ⚠️ 见**发现 4**（几何重叠；物理孔未取证） |
| B5 | 滚到底最后一行不被手势条盖住 | ✅ 末行距屏底 8 CSS px；沉浸式下**不画**手势条 |
| A1 | `stty size` 与可见区一致 | ❌ **44 行 vs 可见 28.4 行** |
| A2 | 首行提示符不被压 | ✅ 提示符本身可见（首两行落在挖孔带里，见发现 4） |
| A3 | 末行不被导航栏/手势条盖住 | ⚠️ 终端自己的末行（y 2838–2904）在导航栏区（2940 起）之上 ✅；**附加键栏第二排落在导航栏区里**，手势条压在键标上 |
| A4 | `seq 1 100` + `clear` 不乱 | ✅ 清屏后首行干净、不叠字 |
| A5 | 键盘弹出/收起 → 行列数跟着变 | ❌ **不变**：`mInputShown` true/false 两种状态 `stty size` 都是 `44 42`，`TuiActivity` 窗口 frame 始终 `[0,0][1440,3120]` |
| A6 | 与 targetSdk 34 旧包对比 | ✅ 做了，**结论被它翻过来**（见 A/B 表） |
| D1 | 网页能开（冷启动） | ✅ `分支2 冷启动重连` → `autoFetchToken: SUCCESS` → `onPageFinished: ack=true` |
| D2 | 终端能连 | ✅ `xsj@rasp:~$` 彩色提示符 |
| D3 | 会话日志导出落盘 | – 手机端**无入口**：插件有意隐藏了会话头那枚「⋯」（§五），App 侧 `enqueueDownload` 因此不可达 |
| E3 | 前台一轮结束不打扰 | ✅ `一轮生成结束…前台=true` → `App 在前台，不发通知`（两次） |
| E1/E2 | 后台短退/长退仍收到通知 | ✅ **E2 通过**：退后台 **7 分 21 秒**（页面侧心跳早已停）后一轮结束 —— `DshNotifier: 已发任务完成通知：查看 dsh hand 项目`（17:14:53），`dumpsys notification` 里 id=2 / channel `dsh-turn-hi` / `AUTO_CANCEL` 在栏，用户侧确认**弹了横幅**；E1（<30s）未单独验 |

#### A/B：targetSdk 34 vs 35

装法：CI 两版 release 包同 versionCode 41、同签名，`adb install -r -d` 覆盖装，**数据保留**。

| 测量项（软键盘弹出时） | targetSdk 34 | targetSdk 35 |
|---|---|---|
| `stty size` | **45 × 42** | **44 × 42** |
| `TuiActivity` 窗口 frame | `[0,129][1440,3120]` | `[0,0][1440,3120]` |
| 附加键栏 a11y bounds | `y 1688–1875`（键盘上沿 1875） | `y 2933–3120`（键盘上沿 1874） |
| 终端首行 | y=154（状态栏带之下） | y=25（**进了状态栏/挖孔带**） |

**`stty size` 与可见区不一致两版都有** —— 既有缺陷，不是这次升出来的。
这次升 35 真正改掉的是**附加键栏的位置**。

#### 发现 1（本次升级引入）：软键盘一弹，附加键栏整条被盖住

34 上 ESC/TAB/CTRL/方向键浮在键盘上方（a11y `y 1688–1875`，贴着键盘上沿）；
35 上同一批节点跑到 `y 2933–3120`，而**键盘从 1874 开始盖** —— 整条看不见、也点不到。
两版节点数一样（各 7 个），所以变的是**位置**，不是键栏没建。
用户侧后果：打字时用不了 ESC / TAB / CTRL / 方向键。

> 根因见下方「2026-09-22 交叉验证」：**同一个包在 Android 13 上这条不出现**（附加键栏完整可见）。

#### 发现 2（既有，不是 35 引入）：PTY 不跟软键盘

清单里是 `adjustResize`，`mAttrs` 也是 `adjust=resize`，但 `mInputShown` 从 false 变 true
时窗口 frame 一动不动，`stty size` 也不变。终端 44 行（2904px），可见只有 28.4 行（键盘上沿 1874px）：
`seq 1 40` 之后**第 26 行往后连同提示符全在键盘底下**（`16-seq40.png`）。
`TuiActivity` 不消费任何 IME inset —— 与本节开头预警的「字符网格被盖住就是真的看不见」同一件事，
只是盖它的是键盘而不是系统栏。

> 根因见下方「2026-09-22 交叉验证」：**同一个包在 Android 13 上 PTY 精确跟随键盘**（39 → 21 行，
> 收缩量 = 键盘高度 981px），所以这条是 API 35+ 强制 edge-to-edge 的后果，不是行列计算写错了。

#### 发现 3：长熄屏后 dbclient 会消失（0.1.8 起一直没验的那条）

- 熄屏 **12s**：`mWakefulness=Dozing`，dbclient 仍在（1 个），回前台 `vsysui` 仍 `0x1706` ✅
  （这一档连 `onPause` 都没触发 —— 三星 AOD 下 Activity 没停）
- 熄屏 **60s**：**dbclient 归零**（应用进程还在、前台服务还在），锁屏 `showing=true`
- 解锁回前台：watchdog 自动重建 —— `dbclient tune #1` → `tunnel state: connected` →
  `ensureTunnel: 已建立 http://127.0.0.1:3080` → `rebuildTunnel: 同 origin → 不重载页面`，
  隧道不变量脚本 5/5 全绿

即**「前台服务保活」没保住子进程，但恢复路径是好的**（用户侧：回前台约 1s 后页面照常用）。
要复现「熄屏冻结」那条链，熄屏时长得 ≥60s。

#### 发现 4：终端首行进了挖孔带（35 起窗口从 y=0 开始）

34 上终端网格从 y=129 起（状态栏之下），35 上从 y=0 起 —— 第 0 行（0–66）与第 1 行（66–132）
落在系统声明的挖孔带（`insets=Rect(0,128,0,0)`，孔 `Rect(686,0 - 754,128)`）里。
宿主那条 mode 图标同理：a11y 实测 `[701,75][757,135]`，与挖孔矩形重叠 53×53 设备px。

> ⚠️ **系统给的是包围盒，不等于物理孔的可见范围**，改不改要由「真机看着挡没挡」定
> —— 本次没取到物理孔几何（adb 拿不到，截图里孔不是画出来的）。

#### 发现 5：右侧栏文件面板没有可用出口（与 rev 1.0.49–1.0.52 的探针同一条）

面板打开后 `文件 关闭 [33,165][416,277]`、`收起右侧边栏 [1308,165][1421,277]`（a11y 都标
`clickable=true`），但**实测点不动**：插件自己的点击追踪在 y=57 这条带上从 x=20 打到 x=364，
`at` 全是 `div._tabStrip_17p4l_156`（`atTop=35 atH=38`）—— **标签条压在按钮上面吃掉了这一笔**。
面板是全屏（`P3OORG_panel` 铺满 832px），App 头部那两个抽屉开关也在它下面：
打开文件面板后**没有任何可点的出口**，BACK 还会直接回连接屏（面板仍开着）。

**2026-09-22 追加（A13 交叉验证 + 出口穷举）**：

- 同一现象在 SM-G7810（Android 13，360 CSS px 宽）上**逐字复现**，命中的还是
  `div._tabStrip_17p4l_156`（`atTop=30 atH=38`；面板自己的标签行在 CSS `y 40–68`）→
  **与屏宽密度、与 API 级别都无关**。
- **出口穷举**（全部在 A13 真机实测）：

  | 尝试 | 结果 |
  |---|---|
  | 点 `文件 关闭` / `收起右侧边栏`（a11y 都 `clickable=true`） | ❌ 被标签条吃掉 |
  | `input keyevent 111`（ESC） | ❌ 无效 |
  | `input keycombination 113 30`（Ctrl+B）、`113 8`（Ctrl+H） | ❌ 无效 |
  | BACK | 回连接屏，**面板仍开着**；再进网页还是它 |
  | 断开连接 → 重新连接（整页重载） | ❌ **面板照旧**，重载前后截图**逐像素相同**（宿主把面板状态持久化了） |
  | App 内「诊断」视图 | 只有「关闭」，没有清页面状态的动作 |

  即：**打开右侧文件面板后 App 内没有任何恢复路径**，只剩清应用数据或宿主侧修。
- 因此 **B3（左抽屉）在 A13 上没能复测** —— App 头部那枚 `打开目录 [36,42][120,126]` 就在面板底下。
- ✅ **竖屏之外的出口：横屏**。转横屏后宿主把面板收成右侧一列，标签条不再压在上面，
  `收起右侧边栏` 落回面板自己手里 —— 点击追踪从 `div._tabStrip_17p4l_156` 变成
...
```sh
D=192.168.0.175:38467
adb -s $D shell input keyevent 4                 # 回连接屏（隧道保留）
adb -s $D shell input tap 1054 2558              # 「打开终端」分段
adb -s $D shell input tap 720 2790               # 主按钮
adb -s $D shell input tap 720 1500               # 唤出软键盘
adb -s $D shell input text 'stty%ssize'; adb -s $D shell input keyevent 66
adb -s $D shell 'dumpsys window windows' | awk '/TuiActivity}/{f=1} f&&/Frames:/{print; exit}'
adb -s $D shell 'uiautomator dump /sdcard/ui.xml' && adb -s $D pull /sdcard/ui.xml .
```

#### 本次没验到的

- **E1**：后台短退（<30s）一轮结束 —— 只验了 E2 那条更难的（>6min、页面心跳已停）路
- **C2**：手势导航下「边缘上滑只出临时系统栏」这一档，没找到不误触的滑法
- **物理挖孔**：包围盒有了，物理孔可见范围取不到

### 2026-09-22 交叉验证：同一个包装在 Android 13 上（SM-G7810 / API 33）

发现 1、2 都指向同一句话「`TuiActivity` 不消费 IME inset」，但在 Android 16 上分不清是
**targetSdk 35 的强制 edge-to-edge** 还是**应用一直就没处理**。换一台 Android 13 装**同一个
APK**（同 versionCode 41 / targetSdk 35），如果缺陷消失，根因就只能是平台在 API 35+ 的强制行为。

装法：原机 0.1.9 / versionCode 36 / targetSdk 34 → `adb install -r -d ts35/app-release.apk`
→ 0.1.14 / versionCode 41 / targetSdk 35，**数据保留**（同一签名）。

| 测量项 | SM-S9280（A16 / API 36） | SM-G7810（A13 / API 33） |
|---|---|---|
| `stty size` 软键盘**收起** | 44 × 42 | **39 × 40** |
| `stty size` 软键盘**弹出** | 44 × 42（**不变**） | **21 × 40**（跟随） |
| 终端视图高度 IME↓ → IME↑ | 3120 → 3120（不变） | 2117 → 1136（**−981**） |
| 键盘高度（设备px） | 3120−1874 = 1246 | 2355−1374 = **981** |
| 附加键栏 a11y bounds（IME↑） | `y 2933–3120`（键盘上沿 1874 → 整条被盖） | `y 1224–1374`（键盘上沿 1374 → **完整可见**） |
| 附加键栏 a11y bounds（IME↓） | — | `y 2205–2355` |
| `TuiActivity` window frame | `[0,0][1440,3120]`（两态相同） | `[0,88][1080,2400]`（两态相同） |
| `mAppBounds` | 满屏 | `[0,88][1080,2355]`（挖孔 88 + 导航条 45 都排除） |
| 终端首行 | IME↑ 时 y=25（**进了挖孔带**） | y=107（在挖孔带之下） |
| 附加键第二行 vs 导航条带 | 落在手势条带里、手势柄压字 | 条带外（`mAppBounds` 已排除 45px），不压 |

#### 根因（现在可以下定论）

**同一个 APK 在 Android 13 上终端完全正常**，两处关键算术都是精确闭合的：

- 内容区 `mAppBounds` = 88→2355 = **2267px** = 终端视图 2117 + 附加键栏 150 ✅
- 键盘弹出后可见内容 = 88→1374 = **1286px** = 终端视图 1136 + 附加键栏 150 ✅
- 收缩量 2355−1374 = **981px = 键盘高度**，PTY 行数 39 → 21（981/53 = 18.5 ≈ 18 行差）✅

也就是说：**Android 13 上系统照旧替应用 resize（`adjust=resize` 生效），应用的行列计算本身是对的**；
Android 15+ 因为 targetSdk ≥ 35 被强制 edge-to-edge，系统不再替应用让位，而 `TuiActivity`
又不消费 `Type.ime()` inset —— 于是键盘**盖**在内容上，附加键栏被顶到屏幕底部（发现 1）、
PTY 保持全高（发现 2）。

推论：
1. 发现 1、2 **不是本次代码退化，是 API 35+ 的强制行为**，修法只有一个：终端自己
   `setDecorFitsSystemWindows(false)` + 消费 `WindowInsetsCompat.Type.ime()`（或改用
   `WindowInsetsAnimationCompat`）把 IME 高度算进终端视图高度，然后 `stty`/SIGWINCH 重新下发。
2. 发现 4（终端首行进挖孔带）同理：顶边也得由应用显式处理 —— Android 13 上窗口自己从 y=88 起，
   Android 16 上从 y=0 起，同一个包两种表现。
3. A3（附加键第二行落在导航条带、手势柄压字）也只在 Android 16 上出现。

#### 其它实测细节（Android 13）

- 字格 **53px 行距 / 27px 字宽**（1080 / 40 列 = 27 ✅）；换算 53/3.0 = **17.7dp**，与 SM-S9280 的
  66/3.75 = 17.6dp 一致 —— 字号没差，差的只是视图高度。
- 末列**没有**被切：整行亮像素 x 5→1076 < 1080。
- 终端**进入即自动弹键盘**（`mInputShown=true`，`mShowExplicitlyRequested=false` → 走的是
  `windowSoftInputMode` 而不是 `showSoftInput()`）。
- 附加键栏第二行在 IME 收起时 a11y bounds 全是 `[0,0][0,0]`（dump 伪影，**不是**点不到）：
  实测 `12345` → 点两次 `←` → 输 `Z` = **`123Z45`**，第二行按键正常工作。
- `⌨` 键（`[945,1224][1080,1374]`，IME↑ 时）可收起键盘；**BACK 在 `TuiActivity` 里直接回连接屏**
  （两台设备一致，键盘弹着也一样）。

#### 交叉验证的复现命令

```sh
D=192.168.0.186:40351
adb -s $D shell input tap 789 1964                 # 「打开终端」分段
adb -s $D shell input tap 540 2135                 # 主按钮（进入即带键盘）
adb -s $D shell input text 'stty%ssize'; adb -s $D shell input keyevent 66   # → 21 40
adb -s $D shell input tap 1012 1299                # ⌨ 收起键盘
adb -s $D shell input text 'stty%ssize'; adb -s $D shell input keyevent 66   # → 39 40
adb -s $D shell 'dumpsys window windows' | grep -m1 -A3 'Frames: parent=\[0,88\]'
```

#### 主界面在 Android 13 上的抽样（B1/B2 通过，发现 5 复现）

| 测量项 | SM-S9280（A16，384 CSS px 宽） | SM-G7810（A13，360 CSS px 宽） |
|---|---|---|
| WebView 视口 | 满屏 `3120` | `[0,0][1080,2267]`（沉浸式从 y=0 起，底部留 133px） |
| 会话头/标签条容器顶边 | 首个内容行 20.5 CSS px | **`[0,36][1080,216]` → 36 设备px = 12.0 CSS px** ✅ 正是判据要的「~12px 压缩」 |
| B3 左抽屉首个可见内容（logo 行） | 62.9 CSS px（挖孔带 0–34.1） | **57.7 CSS px**（挖孔带 0–29.3）—— 两者都在挖孔带之下约 **28.5 CSS px**，与屏宽无关 ✅ |
| 发现 5：面板出口 | 点不动，`at=div._tabStrip_17p4l_156`（`atTop=35 atH=38`） | **同样点不动**，同一元素（`atTop=30 atH=38`）；**横屏可关** |

Android 13 上的点击追踪原文（`adb logcat | grep 点击追踪`）：

```
x:340 y:54 inPanel:true panelTop:0 panelLeft:0
  at:"div._tabStrip_17p4l_156" atTop:30 atH:38 pe:"auto"     ← 点「收起右侧边栏」
x:60  y:54 inPanel:true panelTop:0 panelLeft:0
  at:"div._tabStrip_17p4l_156" atTop:30 atH:38 pe:"auto"     ← 点「文件 关闭」
```
