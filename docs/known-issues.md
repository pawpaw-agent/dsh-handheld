...
| 项 | 结果 |
|---|---|
| 折叠态（配置齐全） | ✓ 状态块「● 已连上电脑」+ `http://127.0.0.1:3080`；摘要 `xsj@192.168.0.135 · 密码`；贴底只有「打开 dsh 网页」+ 一条「断开连接」 |
| 展开态 | ✓（截图）—— 同时暴露一处错位：「收起」比「连接设置」高 15dp（左列 `wrap_content` 只有一行 11sp ≈15dp，右列 48dp 居中）。左列也改成 48dp + 居中后对齐 |
